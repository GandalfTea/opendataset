...
app.post('/create/dataset', upload.single('init'), async (req, res, next) => {
	...
	if(owner_entry['rowCount'] <= 0) {
		...
		let ret = await queryDB(`SELECT EXISTS ( SELECT FROM information_schema.tables WHERE table_name='${name}');`);
		...
		if(FILE_UPLOAD) {
			...
			switch(ret) {
				...
				case csv_mig_errors.SUCCESSFUL_MIGRATION:
					process.stdout.write(` Successful data migration, dataset '${name}' created.`)
