...
router.get("/:dsid/details", async (req, res) => {
  ...
  var ret = await queryDB(
    `SELECT ds_id FROM ds_metadata WHERE ds_name='${dsid}';`
  );
...
router.patch("/:dsid/frontend/description", async (req, res) => {
  ...
  var ret = await queryDB(
    `UPDATE ds_frontend SET description='${new_des}' WHERE ds_id=${dsid};`
  );
  console.log(ret);
  res.status(200);
  res.send("Hello");
...
router.delete("/:dsid", async (req, res) => {
  ...
  if (dsid instanceof String || typeof dsid === "string") {
    ...
    var ret = await queryDB(
      `SELECT ds_name FROM ds_metadata WHERE ds_id='${dsid}';`
    );
